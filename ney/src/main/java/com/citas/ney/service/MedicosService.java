...
public class MedicosService {
    ...
    public MedicoUnicoResponse listarMedicoUnico(Integer id) {
        ModelMedicos medico = medicosRepository.findByUsuarioIdusuario(id).orElseThrow(() -> new RuntimeException("medico no encontrado"));
        return MedicoUnicoResponse.builder()
                .idmedicos(medico.getIdmedicos())
                .idusuario(medico.getUsuario().getIdusuario())
                .idespecilidad(medico.getEspecialidad().getIdespecialidades())
                .especialidad(medico.getEspecialidad().getNombreEspecialidad())
                .nombresMedico(medico.getNombresMedico() + " " + medico.getApellidosMedico())
                .cmpMedico(medico.getCmpMedico())
                .telefonoMedico(medico.getTelefonoMedico())
                .build();
    }
